...
1. 要上傳氧包，請按一下 **上傳套件**.
1. 在「Upload Package（上載包）」對話框中，導航到您在步驟1中下載的Ox包檔案，然後按一下「OK（確定）」。
...
1. 在「安裝套件」對話方塊中，按一下 **安裝**.
1. 安裝完成後，按一下CRX封裝管理器左上角的「首頁」按鈕。
1. 在資產資料夾中選取DITA檔案。
...
1. 在 **文檔類型關聯** 索引標籤，選取 **DITA**，然後按一下 **擴充**.
...
1. 在Oxon XML作者中，按一下&#x200B;**選項** \> **偏好設定**.
1. 在 **文檔類型關聯** 索引標籤，選取 **DITA映射**，然後按一下 **擴充**.
1. 在 **類路徑** 索引標籤中，選取「com.adobe.o2.connector」 **從具有ID的外掛程式使用父類載入器** 下拉式清單。
1. 在 **擴充功能** 頁簽，請進行下列更改：
1. 
   - 按一下 **選擇** 旁邊 **製作擴充功能狀態監聽器** 在 **個別擴充功能** 並在 **類別** 清單。 按一下&#x200B;**「確定」**。
...
### 在Oxon XML作者中開啟檔案 {#id195GHJ0A0UB}
...
1. 輸入搜索查詢\(例如， `introduction`\)，位於AEM指南的Oxo Plugin的搜尋列中。
1. 按一下搜索按鈕或按Enter鍵。
...
1. 選取要套用條件屬性的內容部分。
1. 在Oxon XML作者的「屬性」面板中，按兩下條件屬性。

   ![](images/attribute-panel.png)

1. 在 **可用** 在「編輯屬性」對話框的列中，選擇屬性，然後按一下 **新增**.
