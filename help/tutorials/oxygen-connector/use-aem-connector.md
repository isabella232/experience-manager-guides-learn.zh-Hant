...
1. \（可選\）在簽入對話方塊中，新增註解 **版本註解** 文字方塊。
   ...
   >
   >此註解會顯示在檔案的AEM版本記錄中。
1. 在中新增標籤 **標籤** 文字方塊。 輸入標籤，然後按Enter。 例如， *2307版本*.
如果您的管理員已預先定義標籤清單，並將清單上傳至 `label.json` 檔案，則這些標籤會顯示為下拉式清單。 您可以從下拉式選單中選擇一或多個標籤。
   ![](images/checkin-dropdown-labels.png){width="300" align="left"}
您可以將多個標籤（以逗號分隔）新增至相同版本的主題。  例如， *Adobe*， *AEM*，*指南*
不過，您無法將相同的標籤新增至主題的不同版本。 如果您新增已新增至舊版的標籤，則會新增至最新版本，並從舊版中移除。
