...
AEM Guides為JIRA和SQL (MySQL、PostgreSQL、SQL Server、SQLite)資料庫提供現成的聯結器。 您也可以擴充預設介面來新增其他聯結器。 以下設定可協助您輕鬆新增各種資料來源。 新增後，您即可在網頁編輯器中檢視資料來源。
...
Jira基本驗證的設定檔案範例，其中包含具有「Basic」關鍵字的Token：
...
| templateFolders | 將從中擷取範本的資料夾清單 |

其他欄位是根據選取來執行聯結器的設定類別自訂的。

## 將檔案上傳至AEM中的位置

將檔案上傳至AEM Assets中的某個位置。

例如 `/var/dxml/konnect/jira.json`

## 使用REST API建立設定

您可以使用REST API註冊設定。 如需詳細資訊，請檢視 *REST API可註冊資料來源聯結器* Adobe Experience Manager Guides的API參考中的區段。
