...
1. 使用封裝管理程式來下載/libs/fmdita/config/w2d\_io.xml檔案。

1. 自訂下載的w2d\_io.xml檔案。

1. 在Cloud Manager的Git存放庫中的以下位置新增檔案：
...
如果您是AEM Guidesas a Cloud Service的現有使用者，並且從2021年8月版本升級至2022年1月或更新的版本，請更新指定的屬性，因為只有少數檔案已移動。

>[!NOTE]
>
> 此更新僅適用於您已使用Microsoft Word至DITA轉換工作流程的情況。
...
- 在Cloud Manager Git存放庫中執行必要的變更，因為對於Cloud Service，/apps中的所有檔案都會透過Cloud Manager Git覆蓋。
...
一般而言，轉換程式要求您 [準備InDesign檔案以進行轉換](appendix.md#id195DBF0045Z)[appendix.md\#id195DBF0045Z](appendix.md#id195DBF0045Z) 和 [準備對應檔案以InDesign至DITA移轉](appendix.md#id194AF0003HT)[appendix.md\#id194AF0003HT](appendix.md#id194AF0003HT)，則您必須依照指定的程式執行轉換程式。
...
1. 導覽至「 」中可用的設定檔 `apps` 節點：
   ...
   - 在 `mapStyle` 元素，指定對應檔案的位置，該對應檔案包含InDesign檔案樣式至DITA元素的對應。 預設對應會儲存在檔案中，檔案位於：
     ...
     >[!NOTE]
     >
     > 如需有關結構的詳細資訊， `stmap.adobeidml.xml` 檔案以及如何自訂，請參閱小節 [appendix.md\#id194AF0003HT](appendix.md#id194AF0003HT) 在附錄中。
...
1. 使用封裝管理程式來下載/libs/fmdita/config/h2d\_io.xml檔案。
...
1. *\（可選\）* 您也可以將相關連結區段新增至轉換後的檔案。 執行以下步驟來啟用此功能：
   ...
   1. 使用封裝管理程式下載/libs/fmdita/html2dita/h2d.xsl檔案。
...
AEM Guides可讓您轉換現有的非結構化FrameMaker \(`.fm` 和 `.book`\)將檔案匯入DITA檔案。 第一步是使用FrameMaker建立樣式對應，並將這些設定儲存至.sts檔案中。 接下來，如果您使用自訂DITA，則可以在以下位置將自訂元素與來源FrameMaker格式對應： `ditaElems.xml` 檔案。 例如，如果您建立了自訂元素，命名為 `impnote` 若要處理所有重要附註，您可在以下位置定義此自訂元素： `ditaElems.xml` 檔案。 定義此自訂元素後，轉換FrameMaker檔案包含下列專案時，AEM Guides不會引發錯誤 `impnote` 元素。
...
1. 使用封裝管理程式下載/libs/fmdita/config/ditaElems.xml檔案。
...
1. 使用封裝管理程式下載/libs/fmdita/config/XSLConfig.xml檔案。

1. 在Cloud Manager的Git存放庫中的以下位置建立XSLConfig.xml檔案的副本：
