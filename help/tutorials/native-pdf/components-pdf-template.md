...
雖然CSS檔案是針對內容和版面配置而提供的，但您可以建立新的樣式表以將多個自訂套用至特定樣式型別，然後可套用至目標元件。 依預設，範例CSS檔案是隨產品提供的。 這些CSS檔案旨在協助您跨內容和版面組織樣式資訊。 您可以選擇將這些樣式合併到單一CSS檔案或多個檔案中。
...
通常，當您想要關聯任何DITA元素的自訂樣式時，會建立新樣式。 若要讓這類自訂樣式生效，您必須確保將樣式的類別名稱與DITA元素的outputclass屬性產生關聯。


若要建立新樣式，請遵循下列步驟：
1. 以滑鼠右鍵按一下任一樣式，然後從快顯選單中選擇「新樣式」。
...
* **索引和字彙表**：如果您已設計「索引」頁面配置，請將其對應至「索引」選項。 如果您有「字彙表」頁面配置，則將其對應至「字彙表」選項。
...
* 引用
  ...
  <img src="assets/page-order-advance-settings.png" alt="頁面順序" width="550">

  如果不想在PDF輸出中顯示特定截面，可以關閉切換開關來隱藏它。

  您也可以定義這些區段在PDF中產生的順序。 若要變更這些區段的預設順序，請選取虛線以將區段拖放到所需位置。
...
.
**章節與主題** 配置一律已啟用且 **字彙表** 依預設，配置一律為停用。 您無法將其切換。
