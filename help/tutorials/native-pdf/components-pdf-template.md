...
description: 瞭解PDF模板的各個元件以及如何自定義和配置它們。
...
### 建立新頁面佈局 {#create-page-layout}

>[!NOTE]
>
>有些頁面佈局示例是出廠時提供的。 您可以自定義這些頁面佈局或建立新頁面佈局。
...
1. 開啟要使用的模板。
   >[!NOTE]
   >
   >可以通過按兩下模板名稱或按一下模板名稱旁邊的>表徵圖來開啟模板。
...
1. 指定新頁面佈局的名稱。
   >[!NOTE]
   >
   >在命名頁面佈局時避免使用任何特殊字元。 名稱中的空格將替換為下划線「_」。
1. 按一下 **完成**。

   新佈局將建立並添加到「頁面佈局」下。

### 複製頁面佈局 {#duplicate-page-layout}
...
1. 完成所需更改後，按一下 *全部保存* 或 `Crl+S`)。
   ...
   有關定義單個佈局元素（如頁眉、頁腳、頁碼、標題等）的詳細資訊，請參閱 [設計頁面佈局](design-page-layout.md)。
...
有關使用最常見樣式的詳細資訊，請參閱 [使用常用內容樣式](stylesheet.md)
...
1. 懸停在「資源」資料夾頁籤上，然後按一下（「選項」表徵圖）。..並選擇「導入」。
   ...
   >[!NOTE]
   >
   >無法更改上載資產的路徑。 預設情況下，所有資產都儲存在 `/content/dam/dita-templates/pdf/<PDF-template-name>` 的子菜單。
...
有關應用目錄結構和樣式標題級別的資訊，請參閱 [添加章節TOC](design-page-layout.md#add-chapter-toc)。

   >[!NOTE]
   >
   >如果您是CSS開發人員，則也可以直接在CSS檔案中定義引線格式。
* **使用表繼續標籤**:選擇此選項可定義跨多頁的長表的標籤。 <!--For more information on using table continuation markers, see Use table continuation markers.-->

**頁面佈局**

「頁面佈局」設定允許您完全控制指定要用於文檔特定部分的頁面佈局。 例如，要為目錄選擇佈局，請按一下「目錄」(TOC)欄位下的下拉菜單，然後選擇您設計為生成目錄的佈局。

「頁面佈局」(Page Layout)部分下提供以下設定：

<img src="assets/template-page-layout.png" alt="頁面佈局" width="550">


**預設頁面佈局**:選擇頁面佈局，該佈局用作PDF中所有頁面的預設佈局。 這是應用於尚未建立專用頁面佈局的部分或主題的基本頁面佈局。

**不同節的頁面佈局**:您可以選擇使用以下PDF輸出部分映射頁面佈局：
* 目錄
* 數字清單
* 表清單
* 章節和主題
...
如果不想在PDF輸出中顯示特定節，則可以通過關閉切換開關來禁用該節。

**章節和主題** 佈局始終處於啟用狀態， **辭彙表** 預設情況下，佈局始終處於禁用狀態。 不能切換。

您還可以定義在PDF中生成這些不同節的順序。 如果要更改這些頁面的預設順序，請將滑鼠懸停在頁面佈局的左側，然後按一下虛線表徵圖，將頁面佈局拖放到所需位置。
您的PDF將按您在此處組織的順序包含已啟用的頁面佈局。

**空頁面的頁面佈局**:您還可以為空頁面指定頁面佈局。 所選佈局將應用於所有空頁面。 例如，如果為所有空頁面設計了「空白」頁面佈局，則選擇 **空白** 下拉清單中的「空白」頁面，且文檔中的所有空頁面都將具有「空白」頁面佈局。

**封面和後頁**:如果您設計了封面佈局，則將其映射到 **封面** 的雙曲餘切值。 同樣，如果您有後頁佈局，則將其映射到 **後頁** 的雙曲餘切值。 如果未建立封面或封底頁面佈局，則應用預設頁面佈局。

>[!NOTE]
>
>書籤設定優先於頁面佈局設定。

有關頁面佈局的詳細資訊，請參見 [設計頁面佈局](design-page-layout.md)。
...
* **顏色空間**:您可以選擇RGB或CMYK色彩空間來打印PDF文檔。 選擇RGB以數字顯示生成的PDF和CMYK以進行物理打印。 文檔中定義的顏色將轉換為所選顏色空間。
   >[!NOTE]
   >
   >如果使用CMYK顏色空間，則PDF/A建立需要ICC顏色配置檔案。

   <!--For more information on applying these print settings, see *Printing preferences*.-->

**交叉引用**

使用「交叉引用」(Cross-reference)頁籤定義交叉引用如何發佈PDF。 可以設定主題標題、表、圖等的交叉引用格式。 <!--For more information, see *Format cross-references*.-->
