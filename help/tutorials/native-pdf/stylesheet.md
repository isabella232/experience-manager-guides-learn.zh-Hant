...
# 使用常用內容樣式 {#work-with-common-styles}
...
使用結構化DITA內容時，大多數DITA元素的樣式映射都位於預設樣式表中。 如果您正在使用標準DITA元素，則可以通過直接更改樣式定義來更改其外觀。 這些樣式定義可在「其它樣式」(Other Style)類別下使用。 有關詳細資訊，請參閱 [使用其他樣式](#other-styles) 以後的話題。
...
>[!NOTE]
>
>您可以建立自己的自定義標題樣式，這些樣式可以使用輸出類在內容中使用。 有關詳細資訊，請參閱中的步驟4 [使用頁面方向和視圖旋轉](design-page-layout.md#page-orientation-rotation) 示例。

### 建立自定義章節級標題 {#create-chapter-level-heading}
...
1. 開啟所需的樣式表進行編輯。
   >[!NOTE]
   >
   >請參閱 [自定義預定義或新樣式](components-pdf-template.md#customize-style) 用於開啟樣式表以進行自定義或編輯的部分。
...
1. 選擇 `span.BoldItalic` 從 **字元樣式** ，並設定以下屬性：
   ...
   * **字型**:可以從此部分自定義所有與字型相關的屬性。 預設情況下，有些字型與產品捆綁在一起。 可以為字元樣式選擇所需的字型。 例如，將「字型系列」設定為 *襯線，* 選擇 *粗體* 和 *斜體* 的子菜單。 您還可以自定義其他字型屬性，如「字型粗細」（如粗體、淺色）、「文本修飾」（如下划線、上線）、「字型大小」、「字型顏色」、「文本對齊」等。
      ...
      >[!NOTE]
      您還可以將字型添加到模板中，這些字型儲存在模板的「資源」部分中。 有關添加字型和使用資源的詳細資訊，請參閱 [使用資源](components-pdf-template.md#work-with-resources)。
