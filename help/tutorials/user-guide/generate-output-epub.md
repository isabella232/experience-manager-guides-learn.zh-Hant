...
若要開啟EPUB的輸出預設集，請按一下DITA map檔案，然後按一下輸出預設集，再按一下EPUB選項。 下列選項可用於「EPUB輸出」：
...
| 檔案名稱 | 指定要用來儲存EPUB輸出的檔案名稱。<br><br>**注意**：如果您未提供檔案名稱，則會使用DITA map的標題來產生最終EPUB輸出檔案名稱。 如果地圖沒有標題，則會使用DITA map的檔案名稱來命名最終EPUB輸出。 系統會使用系統中設定的規則來清除檔案名稱，以處理任何無效字元。 |
