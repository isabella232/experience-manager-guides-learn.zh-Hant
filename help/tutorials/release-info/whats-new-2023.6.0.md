...
AEM Guides可讓您檢查技術檔案的整體完整性，以及從網頁編輯器產生報告。 現在於2023年6月發行AEM Guides，提供您檢視及修正失效連結的功能。 這是一個實用的報表，可協助您管理中斷的連結。 您可以輕鬆檢視DITA map中存在的中斷連結，也可以加以修正。
...
AEM Guides提供語言變數的支援。 您可以使用語言變數來定義PDF輸出中現成可用標籤（例如「注意」、「警告」和「警告」或靜態文字）的當地語系化版本。
您可以將語言變數或當地語系化版本的標籤新增至PDF輸出和輸出範本中的適當區段。
...
如果您想要以各種語言建立PDF輸出，則必須建立包含每種語言本地化文字的不同PDF範本。 現在使用語言變數功能，您只需要建立範本一次。 然後，對於您需要本地化的任何靜態文字，您可以建立對應的語言變數並在範本中使用它們。
您可以為較長的文字建立語言變數，例如整個句子甚至段落。 您也可以套用樣式，並使用HTML標示來格式化這些語言變數。
